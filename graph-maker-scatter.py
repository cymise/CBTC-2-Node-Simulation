...
plt.scatter(train_point_x[::50], ap1_snr[::50], s = 0.1, c = "black", alpha = 0.9)
plt.title("AP 1 SNR scatter of simulation environment\nLog-Distance Pathloss")
...
plt.grid()
plt.show()
#plt.savefig('./graph/fig_snr_ldpl_t_1.png', dpi=300)


plt.figure()
plt.scatter(train_point_x[::50], ap2_snr[::50], s = 0.1, c = "black", alpha = 0.9)
plt.title("AP 2 SNR scatter of simulation environment\nLog-Distance Pathloss")
plt.xlabel("Distance of train from simulation origin")
plt.ylabel("SNR (dB)")
plt.legend()
plt.axis([0, 80, 0, 60])
plt.grid()
plt.show()
#plt.savefig('./graph/fig_snr_ldpl_t_2.png', dpi=300)
