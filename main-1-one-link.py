...
frame_list = [{"probe request":400}, {"probe response":400}, {"authentication request":400, "ack":14}, {"authentication response":400, "ack":14}, {"association request":400, "ack":14}, {"association response":400, "ack":14}]#보낼 프레임 목록. 프레임 이름, 프레임 길이(바이트 단위)를 딕셔너리로 저장. 각 딕셔너리 간에는 백오프 과정 수행. e.g. [{"ProbeReq":100, "Ack": 50}, {"ProbeResp": 100, "Ack": 50}]
...
def calc_ho_delay():
    ...
    #print(frame_log)
    print(delay_log)
    filename = "./save/"+"SC1_"+pathloss_marker+datetime.today().strftime("%d_%H_%M") + ".txt"
